...
AC_MSG_CHECKING([for PETSc version 3.7 or 3.8])
...
#if ((PETSC_VERSION_GE(3,7,0) && PETSC_VERSION_LT(3,9,0)) || !PETSC_VERSION_RELEASE)
#else
asdf
#endif
...
if test "$PETSC_VERSION_VALID" = no; then
  AC_MSG_ERROR([invalid PETSc version detected: please use PETSc 3.7 or 3.8.2])
fi
